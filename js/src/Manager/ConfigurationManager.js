...
export default class ConfigurationManager {
  ...
  initConfiguration() {
    ...
    if (configurationValue.length <= 0) return new ExceptionManager().throwException("Configuration is empty.", null, null);

    try {
      const parsedConfiguration = JSON.parse(configurationValue.trim());
      if (typeof parsedConfiguration !== 'object') {
        return new ExceptionManager().throwException("Parsed configuration is not an object.", null, null);
      }
      ...
      return new ExceptionManager().throwException("Tried to parse invalid JSON configuration.", e, null);
    }
  }
}
