...
const getFirstBySelector = (selector, root = null) => {
  if (root !== null) {
    ...
    return element ? element : new ExceptionManager().throwException(`Element with selector "${selector}" not found in the provided root.`, null, null);
  } else {
    const element = document.querySelectorAll(selector)[0];
    return element ? element : new ExceptionManager().throwException(`Element with selector "${selector}" not found.`, null, null);
  }
}
...
const getAllBySelector = (selector, root = null) => {
  if (root !== null) {
    ...
    return elements ? elements : new ExceptionManager().throwException(`Element with selector "${selector}" not found in the provided root.`, null, null);
  } else {
    const elements = document.querySelectorAll(selector);
    return elements ? elements : new ExceptionManager().throwException(`Element with selector "${selector}" not found.`, null, null);
  }
}
...
const swapActiveClassName = (disableFor = [], enableFor = [], className) => {
  disableFor.forEach((HTMLElem, _) => {
    if (HTMLElem.classList) {
      HTMLElem.classList.contains(className) ? HTMLElem.classList.remove(className) : null;
    }
  });

  enableFor.forEach((HTMLElem, _) => {
    if (HTMLElem.classList) {
      !HTMLElem.classList.contains(className) ? HTMLElem.classList.add(className) : null;
    }
  });
}
